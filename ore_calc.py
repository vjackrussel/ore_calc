from PyQt5.QtWidgets import QMainWindow, QApplication, QWidget, QPushButton, QAction, QPlainTextEdit, QMessageBox, QTableView, QHeaderView, QLabel
...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.button = QPushButton("Calculate Value", self)
        self.button.move(20, 560)
        ...
        self.orelabel.setToolTip("Default value of 22.4 m3/s based on a max skill Covetor using 2x Strip Miner I")
        self.icelabel = QLabel(self)
        self.icelabel.setText("Enter your ice m3/s:")
        self.icelabel.move(1200, 560)
        self.icelabel.setToolTip("Default value of 29.8 m3/s based on a max skill Covetor using 2x Ice Harvester II")
        ...
        self.mercoxitlabel.setToolTip("Default value of 10.4 m3/s based on a max skill Covetor using 2x Modulated Deep Core Strip Miner II with no charge")
    ...
    def on_click(self):
        ...
        try:
            tableValue["Compressed Units"] = tableValue["Compressed Units"].map(lambda x: x.replace(',',''))
        ...
        for i, row in tableValue.iterrows():
            ...
            try:
                ...
                unit_value, date_obtained = getMarketValue(ore_data_dict[i]["type_id"])
            ...
            if ore_data_dict[i]["compression_ratio"] == 1:
                mined_compressed_units = ice_m3_per_sec / ore_data_dict[i]["volume"] * 3600
            elif "Mercoxit" in i:
                mined_compressed_units = mercoxit_m3_per_sec / ore_data_dict[i]["volume"] * 36
            else:
                mined_compressed_units = ore_m3_per_sec / ore_data_dict[i]["volume"] * 36
            isk_per_hour = mined_compressed_units * unit_value
            tableValue.at[i, "ISK/Hr"] = isk_per_hour
            tableValue.at[i, "Total Value"] = tableValue.at[i, "Compressed Units"] * unit_value
        tableValue = tableValue.round({"Unit Volume":2, "Total Volume":1, "Unit Value":2,"ISK/Hr":2, "Total Value":2})
